...
export class DelayTasks {
    ...
    constructor(settings: { redis?: Redis | RedisOptions, id?: string, callback?: CallbackFunctionVariadic, verbose?: boolean, options?: { pollIntervalMs?: number } } = { verbose: false }) {
        ...
        const isRedis = (settings!.redis as Redis).connect !== undefined;
        if (isRedis) {
            this.redis = settings.redis as Redis;
        } else if (typeof settings.redis === 'object') {
            this.redis = new Redis(settings.redis as RedisOptions);
        } else {
    ...
    poll() {
        ...
        return new Promise((resolve, reject) => {
            this.redis.watch(this.redisKey).then(async (_) => {
                try {
                    const tasks = await this.redis.zrangebyscore(this.redisKey, 0, now);
                    if (tasks.length > 0) {
                        try {
                            const results = await this.redis.multi().zremrangebyscore(this.redisKey, 0, now).exec();
     ...
     */
    addToRedis(delayedTime: number, task: string): Promise<number> {
        return new Promise((resolve, reject) => {
     ...
     * Add a delayed task.
     */
    async add(delayMs: number, data: object): Promise<string> {
        // Validate `delayMs`
    ...
    private async getNextTaskTime(): Promise<number|null> {
